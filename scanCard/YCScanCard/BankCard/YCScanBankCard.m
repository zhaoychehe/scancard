...
- (void)parseBankImageBuffer:(CVImageBufferRef)imageBuffer {
    ...
    CGRect effectRect = [YCCardRectManager getEffectImageRect:size];
    CGRect rect = [YCCardRectManager getGuideFrame:effectRect];
    ...
    if(resultLen > 0) {
        ...
        int charCount = [YCCardRectManager docode:result len:resultLen];
        if(charCount > 0) {
            CGRect subRect = [YCCardRectManager getCorpCardRect:width height:height guideRect:rect charCount:charCount];
            self.isHasResult = YES;
            UIImage *image = [UIImage getImageStream:imageBuffer];
            __block UIImage *subImg = [UIImage getSubImage:subRect inImage:image];
            
            char *numbers = [YCCardRectManager getNumbers];
            
            NSString *numberStr = [NSString stringWithCString:numbers encoding:NSASCIIStringEncoding];
            NSString *bank = [YCBankCardSearch getBankNameByBin:numbers count:charCount];
            
            NSLog(@"%@",numberStr);
            NSLog(@"%@",bank);
            //            NSLog(@"%@",)
            
            YCBankCardModel *model = [YCBankCardModel new];
...
- (void)captureOutput:(AVCaptureOutput *)captureOutput didOutputSampleBuffer:(CMSampleBufferRef)sampleBuffer fromConnection:(AVCaptureConnection *)connection {
    ...
    if ([captureOutput isEqual:self.videoDataOutput]) {
        if(self.isInProcessing == NO) {
            if (self.receiveSubjectBlock) {
                self.receiveSubjectBlock((__bridge id)(imageBuffer));
            }
